...
# ssh-agent plugin: у нас нет id_rsa/id_ed25519 со стандартными именами,
# поэтому явно перечисляем ключи, которые нужно подхватывать при старте шелла.
# Список identities обязательно задавать ДО source $ZSH/oh-my-zsh.sh.
zstyle :omz:plugins:ssh-agent identities adminvps github@home macos@home openwrt@home mikrotiks@work
# Хранить/подтягивать passphrase из macOS Keychain, чтобы не вводить её каждый раз
